...
VecAddLayer::VecAddLayer(base::DeviceType device_type)
  ...
  reset_input_size(2);    // 重置输入张量大小为2
  reset_output_size(1);   // 重置输出张量大小为1
}
...
base::Status VecAddLayer::check() const {
  ...
  base::Status status;
  status = check_tensor_with_dim(input1, device_type_, data_type_, size); // 检查输入张量1的维度
  if (!status) {
...
base::Status VecAddLayer::forward() {
  ...
  auto add_kernel = kernel::get_add_kernel(device_type_); // 获取加法核函数
  add_kernel(input1, input2, output, cuda_config_ ? cuda_config_->stream : nullptr);  // 计算
  return base::error::Success();
}
