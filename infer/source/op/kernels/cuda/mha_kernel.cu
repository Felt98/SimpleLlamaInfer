...
// pos: 当前token的索引
// query：(batch, seqlen, heads, head_dim)，batch=1， seqlen=1
// score：(batch, heads, seqlen, seqlen)
// output：(batch, seqlen, heads, head_dim)
// key_cache：(batch, cache_len+seqlen, heads, head_dim) , cache_len是历史的输出，Decorder会将历史输出作为输入，所以key_cache的维度是cache_len+seqlen
// value_cache：(batch, cache_len+seqlen, heads, head_dim) , cache_len是历史的输出，Decorder会将历史输出作为输入，所以value_cache的维度是cache_len+seqlen
// kv_dim：head_dim
// kv_mul：head_num
// layer_offset：layer_index * seq_len * kv_dim， 定位当前transformer的第layer_index层（每个transformer层都有一个kv_cache）
__global__ void multi_head_attention_kernel(int32_t pos, int32_t seq_len, float* query,
  ...
  float* query_head = query + head * head_size;   // query_head: (1，head_dim),当前头对应的query
  ...
  float* score_head = score_ptr + head * seq_len;  // score_head: (1, seqlen)，当前头对应的score
  ...
  // 计算自注意力分数 query_head @ key_head
  for (int t = threadIdx.x; t <= pos; t += blockDim.x) {
    // key_cache + layer_offset ：当前transformer层的kv_cache的偏移量
    float* key_head = key_cache + layer_offset + t * kv_dim + head_offset;   // key_head: (cache_len+seqlen, head_dim)
...
void mha_kernel_cu(int32_t pos, int32_t head_num, int32_t layer_index, int32_t seq_len,
  ...
  // 一个block处理一个head，一个thread处理一个head_size
