...
base::Status MultiHeadAttention::forward() {
  ...
  const tensor::Tensor& mha_out = this->get_output(0);  // 输出张量
  const tensor::Tensor& query_tensor = this->get_input(0);  // query张量
  const tensor::Tensor& score_tensor = this->get_input(1);  // score张量
  const tensor::Tensor& key_cache_tensor = this->get_input(2);  // key缓存张量
  const tensor::Tensor& value_cache_tensor = this->get_input(3);  // value缓存张量
