...
SpeEncodeLayer::SpeEncodeLayer(std::string token_model_path, bool has_bos, bool has_eos)
  ...
  spe = std::make_unique<sentencepiece::SentencePieceProcessor>();  // 创建句柄

  // 加载词表，token_model_path_为词表路径
...
std::vector<int32_t> SpeEncodeLayer::encode(const std::string& sentence) const {
  CHECK(spe != nullptr);
  // sentencepiece库的EncodeAsIds函数，将句子编码为id列表
  std::vector<int32_t> input_ids = spe->EncodeAsIds(sentence);

  // 如果需要添加bos_id，则添加到input_ids的头部
  if (has_bos_) {
...
int32_t SpeEncodeLayer::vocab_size() const {
  CHECK(spe != nullptr);
  return spe->GetPieceSize(); // 获取词表大小
}
...
std::vector<int32_t> BpeEncodeLayer::encode(const std::string& sentence) const {
  ...
  replacements[" "] = "Ġ";
  std::string s = absl::StrReplaceAll(sentence, replacements);  // 将句子中的空格替换为Ġ

  // 调用tiktoken库的encode函数，将句子编码为id列表
